...
extern MPROG_DATA	*new_mprog( void );
...
/*  {   command		function	}, */
    ...
    {   "chance",		medit_randpct   },
    {   "number",		medit_randnum   },
    {   "types",		medit_randtype  },
    {   "?",		show_help	},
    {   "version",	show_version	},
    {	"",		0,		}
};
...
const struct olc_cmd_type mpedit_table[] =
 ...
 /*  {   command		function	}, */  
     ...
     {	"program",	rpedit_program	},
     {	"show",		rpedit_show	},
     {	"trigger",	rpedit_trigger	},
      {   "?",		show_help	},
      {   "version",	show_version	},
      {	"",		0,		}
  };
 ...
 /*  {   command		function	}, */
     ...
     {	"program",	opedit_program	},
     {	"show",		opedit_show	},
     {	"trigger",	opedit_trigger	},
     {   "?",		show_help	},
     {   "version",	show_version	},
     {	"",		0,		}
 };
...
char arg[MAX_INPUT_LENGTH];
   AREA_DATA *pArea;
	...
	{
	update_last("aedit:", command, argument);
...
char arg[MAX_INPUT_LENGTH];
	...
	{
	update_last("redit:", command, argument);
...
char arg[MAX_INPUT_LENGTH];
	...
	{
	update_last("oedit:", command, argument);
...
void medit( CHAR_DATA *ch, char *argument )
{
   char arg[MAX_INPUT_LENGTH];
	...
	{
	update_last("medit:", command, argument);
...
void mpedit( CHAR_DATA *ch, char *argument )
{
     ...
     {
	send_to_char("You do not have permission to make progs.\n\r", ch);
	return;
     }
 	...
 	{
	update_last("mpedit:", command, argument);
 ...
 void do_mpedit( CHAR_DATA *ch, char *argument )
 {
	...
	return;
 	...
 	if ( !str_cmp( arg1, "create" ) )
 	{
 		...
 		value = atoi( argument ); 		if ( !( cMob = get_mob_index( value ) ) )
 		{
 		...
 		/* Start copying */
 		mprg = pMob->mudprogs; /* allocated by create */
 ...
 /* RoomProg Interpreter, called by do_rpedit. */
 void rpedit( CHAR_DATA *ch, char *argument )
 {
	...
	return;
     ...
     {
 	if( pMobProg->type )
 	    REMOVE_BIT( pRoom->progtypes, pMobProg->type );
         pMobProg->type = value;
 	SET_BIT( pRoom->progtypes, pMobProg->type );
     ...
     /* Search Table and Dispatch Command. */
     for ( cmd = 0; rpedit_table[cmd].name[0] != '\0'; cmd++ )
     {
 ...
 /* Entry point for editing room_prog_data. */
 void do_rpedit( CHAR_DATA *ch, char *argument )
 {
	...
	return;
     ...
     if ( ch->in_room && !IS_BUILDER( ch, ch->in_room->area ) )
     {
 	send_to_char( "RPEdit:  Insufficient security to edit mobprog.\n\r", ch );
 	return;
     }
 	...
 	if( arg2[ 0 ] == '\0' || atoi( arg2 ) == 0 )
 	{
 	    if( !pRoom->mudprogs )
 	    {
 		send_to_char( "RPEdit:  Room has no roomprogs.  Use create.\n\r", ch );
 		return;
 	    }
 	    else
 	    {
 		send_to_char( "RPEdit:  Editing first roomprog.\n\r", ch );
 		value = 1;
 	    }
 	}
 	else if( ( value = atoi( arg2 ) ) > rprog_count( pRoom ) )
 	{
 	    send_to_char( "RPEdit:  Room does not have that many roomprogs.\n\r", ch );
 	    return;
 	}
 	...
 	if ( !str_cmp( arg1, "create" ) )
 	{
 	    ...
 	    if ( arg2[0] == '\0' || value == 0 )
 	    {
 		send_to_char( "Syntax:  edit rprog create vnum [svnum]\n\r", 
 			ch );
 		return;
 	    }
 		...
 		if ( !( pRoom = get_room_index( value ) ) )
 		{
 		    send_to_char( "No destination room exists.\n\r", ch );
 		    return;
 		}
 		...
 		{
 		    send_to_char( "No such source room exists.\n\r", ch );
 		    return;
 		}
 
 		/* Start copying */
 		mprg = pRoom->mudprogs; /* allocated by create */
     ...
     send_to_char( "RPEdit:  There is no default room to edit.\n\r", ch );
     return;
 }
 
 /* ObjectProg Interpreter, called by do_opedit. */
 void opedit( CHAR_DATA *ch, char *argument )
 {
	...
	return;
     ...
     {
 	if( pMobProg->type )
 	    REMOVE_BIT( pObj->progtypes, pMobProg->type );
         pMobProg->type = value;
 	SET_BIT( pObj->progtypes, pMobProg->type );
     ...
     /* Search Table and Dispatch Command. */
     for ( cmd = 0; opedit_table[cmd].name[0] != '\0'; cmd++ )
     {
 ...
 void do_opedit( CHAR_DATA *ch, char *argument )
 {
	...
	return;
     ...
     if ( ch->in_room && !IS_BUILDER( ch, ch->in_room->area ) )
     {
 	send_to_char( "OPEdit:  Insufficient security to edit objprog.\n\r", ch );
 	return;
     }
 	...
 	if( arg2[ 0 ] == '\0' || atoi( arg2 ) == 0 )
 	{
 	    if( !pObj->mudprogs )
 	    {
 		send_to_char( "OPEdit:  Object has no objprogs.  Use create.\n\r", ch );
 		return;
 	    }
 	    else
 	    {
 		send_to_char( "OPEdit:  Editing first objprog.\n\r", ch );
 		value = 1;
 	    }
 	}
 	else if( ( value = atoi( arg2 ) ) > oprog_count( pObj ) )
 	{
 	    send_to_char( "OPEdit:  Object does not have that many objprogs.\n\r", ch );
 	    return;
 	}
 	...
 	if ( !str_cmp( arg1, "create" ) )
 	{
 	    ...
 	    if ( arg2[0] == '\0' || value == 0 )
 	    {
 		send_to_char( "Syntax:  edit oprog create vnum [svnum]\n\r", 
 			ch );
 		return;
 	    }
 		...
 		if ( !( pObj = get_obj_index( value ) ) )
 		{
 		    send_to_char( "No destination object exists.\n\r", ch );
 		    return;
 		}
 		...
 		{
 		    send_to_char( "No such source object exists.\n\r", ch );
 		    return;
 		}
 
 		/* Start copying */
 		mprg = pObj->mudprogs; /* allocated by create */
...
void do_alist( CHAR_DATA *ch, char *argument )
{
    ...
    printf_to_char( ch, "[%3s] [%-27s] (%-5s-%5s) [%-10s] %3s [%-10s]\n\r",
       "Num", "Area Name", "lvnum", "uvnum", "Filename", "Sec", "Builders" );

    for ( pArea = area_first; pArea; pArea = pArea->next )
    {
	printf_to_char( ch, "[%3d] %-29.29s (%-5d-%5d) %-12.12s [%d] [%-10.10s]\n\r",
	     pArea->vnum,
	     &pArea->name[8],
	     pArea->lvnum,
	     pArea->uvnum,
