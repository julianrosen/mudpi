...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
   ...
   if((!IS_NPC(ch) && ch->size < SIZE_LARGE  ))
	...
	{
		send_to_char("You are wielding a two handed weapon! DUH!\r\n",ch);
		return;
	}}}
...
char arg2[MAX_INPUT_LENGTH];
    ...
    if ( !oprog_use_trigger( ch, scroll, NULL, NULL, NULL ) )
      {
...
CHAR_DATA *victim;
	...
	if ( victim != NULL )
	{
    if ( !oprog_use_trigger( ch, wand, NULL, NULL, NULL ) )
      {
...
/* bug fixes in buy function fixed here by Rindar (Ron Cole) */
...
*/
	...
	sprintf(buf2, "WARNING! %s just tried to buy %s %s's!", ch->name, arg, arg2);
	send_wiz(94, buf2);
            ch->reply = keeper;
            return;
        }
