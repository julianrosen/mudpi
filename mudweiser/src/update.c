...
int hit_gain( CHAR_DATA *ch )
{
    ...
    if ( ch->in_room &&
	IS_SET(ch->in_room->room_flags, ROOM_FAST_REGEN))
	gain += number_range(gain, gain * 3 / 2);

    if ( ch->in_room &&
	IS_SET(ch->in_room->room_flags, ROOM_SLOW_REGEN))
...
void mobile_update( void )
{
	...
	if (ch->memory!=NULL)
	 if (get_char_world(ch,ch->memory->name)==NULL) ch->memory=NULL;
	...
	if ( d->connected == CON_PLAYING)
	  {
	     ...
	     for ( raf = ch->in_room->raffect; raf != NULL; raf = raf->next )
	       {
		  if (raf->timer > 0)
		    {
		       if ( raf->type == RAFF_DAMAGE)
			 {
			    dam = number_range(raf->val1, raf->val2);
			    
			    
			    /* if (IS_AFFECTED(ch, AFF_RESIST_FIRE) )
			     * dam -= dam/3; 
			     */
			    
			    
			    raf_damage(ch, dam, raf->val0 ,raf->dam_name);
			    act(raf->room_message, ch,NULL,NULL,TO_ROOM);
			    act(raf->vict_message, ch,NULL,ch,TO_CHAR);
