...
OBJ_DATA * make_random_obj( sh_int level, long possible_type)
{
	...
	if( IS_SET( possible_type, ITEM_RAND_ANY ) )
	{
		...
		{
		case 1: type = ITEM_MAGIC_WEAPON; break; 
		case 2:  
		case 3: type = ITEM_PLAIN_ARMOR; break;
		case 4: 
		case 5: type = ITEM_PLAIN_WEAPON; break;
		case 6: type = ITEM_MAGIC_ARMOR; break;
		case 7: type = ITEM_RAND_RING; break;
		case 8: type = ITEM_RAND_CONTAINER; break;
	...
	switch( type )
	{
	case ITEM_PLAIN_ARMOR: return make_rand_armor( level, FALSE );
	case ITEM_MAGIC_ARMOR: return make_rand_armor( level, TRUE  );
	case ITEM_RAND_RING: return make_rand_ring( level );
/*	case ITEM_MAGIC_ITEM: return make_rand_armor( level, FALSE ); */
	case ITEM_PLAIN_WEAPON: return make_rand_weapon( level, FALSE );
	case ITEM_MAGIC_WEAPON: return make_rand_weapon( level, TRUE  );
	case ITEM_RAND_CONTAINER: return make_rand_bag( level );
...
/* change these to be more normal if you want to have them */
	...
	{ "ultimate", 27, 27, 90},
	{ "kick-ass", 30, 30, 94},
	{ "supercalifragilistdexpealidosiousistic", 30, 30, 98} /* he he... */
};
...
int	get_random_material( sh_int level)
{
	...
	{
		if( rand_material_table[x].level <= level + 5 && rand_material_table[x].level >= level - 5)
		{
			if( lower == -1 )
				lower = x;
			upper = x;
	...
	x = (rand_material_table[lower].level + rand_material_table[upper].level )/2;
	if( x == rand_material_table[lower].level || x == rand_material_table[upper].level )
		{
		material = upper;
		}
	else
	{	
		...
		{
			a=0;
			if(rand_material_table[y].level < x && rand_material_table[y].level > x - 6)	
			...
			else if(rand_material_table[y].level > x && rand_material_table[y].level < x + 6)
				for(z=x+6-rand_material_table[y].level; p < 36 && a < z; p++)
				{
				poss[p]=y;
				a++;
				}
...
OBJ_DATA * make_rand_bag( sh_int level )
{
	...
	{
	sprintf( buf, "%s %s", rand_bagmaterial_table[style].name, rand_bag_table[type].name  );
	...
	rand_bagmaterial_table[style].name[0]=='u' )
		sprintf( buf2, "an %s %s", rand_bagmaterial_table[style].name, rand_bag_table[type].name );
	else
		sprintf( buf2, "a %s %s", rand_bagmaterial_table[style].name, rand_bag_table[type].name );
	...
	rand_bagmaterial_table[style].name[0]=='o' ||
	rand_bagmaterial_table[style].name[0]=='u' )
		sprintf( buf3, "An %s %s lies here.", rand_bagmaterial_table[style].name, rand_bag_table[type].name );
	else
		sprintf( buf3, "A %s %s lies here.", rand_bagmaterial_table[style].name, rand_bag_table[type].name );
	free_string( obj->description );
	obj->description = str_dup( buf3 );
	}
	else
	{
	sprintf( buf, "%s %s", rand_bag_table[type].name, rand_bagmaterial_table[style].name );
	free_string( obj->name );
	obj->name = str_dup( buf );
	...
	rand_bag_table[type].name[0]=='u' )
		sprintf( buf2, "an %s %s", rand_bag_table[type].name, rand_bagmaterial_table[style].name );
	else
		sprintf( buf2, "a %s %s", rand_bag_table[type].name, rand_bagmaterial_table[style].name );
	free_string( obj->short_descr );
	obj->short_descr = str_dup( buf2 );
	if(rand_bag_table[type].name[0]=='a' ||
	rand_bag_table[type].name[0]=='e' ||
	...
	rand_bag_table[type].name[0]=='o' ||
	rand_bag_table[type].name[0]=='u' )
		sprintf( buf3, "An %s %s lies here.", rand_bag_table[type].name, rand_bagmaterial_table[style].name );
	else
		sprintf( buf3, "A %s %s lies here.", rand_bag_table[type].name, rand_bagmaterial_table[style].name );
	free_string( obj->description );
	obj->description = str_dup( buf3 );
	}
	obj->level = number_range( 0, level );
	obj->wear_flags = ITEM_TAKE|rand_bag_table[type].wear_loc;
...
OBJ_DATA * make_rand_weapon( sh_int level, bool ismagic )
{
	...
	sprintf( buf, "%s %s", rand_material_table[material].name, rand_weapon_table[type].name  );
	free_string( obj->name );
	obj->name = str_dup( buf );
	...
	rand_material_table[material].name[0]=='u' )
		sprintf( buf2, "an %s %s", rand_material_table[material].name, rand_weapon_table[type].name );
	else
		  sprintf( buf2, "a %s %s", rand_material_table[material].name, rand_weapon_table[type].name );
	free_string( obj->short_descr );
	obj->short_descr = str_dup( buf2 );
	...
	rand_material_table[material].name[0]=='o' ||
	rand_material_table[material].name[0]=='u' )
		sprintf( buf3, "An %s %s lies here.", rand_material_table[material].name, rand_weapon_table[type].name );
	else
		  sprintf( buf3, "A %s %s lies here.", rand_material_table[material].name, rand_weapon_table[type].name );
	free_string( obj->description );
	obj->description = str_dup( buf3 );
	obj->level = level;
	obj->value[0] = rand_weapon_table[type].weapon_type;
	obj->value[1] = (level/10) + 2;
	obj->value[2] = number_range( 3, 3 + rand_weapon_table[type].damage_mod );
	...
	case 0:
	   paf->location  = attrib_types[number_range(0, TABLESIZE(attrib_types)-1)];
	   paf->modifier  = number_range(1, rand_weapon_table[type].magic_mod );
	   break;
	case 1:
	   paf->location  = power_types[number_range(0, TABLESIZE(power_types)-1)];
	   paf->modifier  = UMAX(1,number_range(level/2, level+rand_material_table[material].magic_mod));
	   break;
	case 2:
	case 3:
	   paf->location  = combat_types[number_range(0, TABLESIZE(combat_types)-1)];
	   paf->modifier  = number_range(1, level/8+1);
	   break;
		}
	if(paf->location==APPLY_AC) paf->modifier *= -1;
	}
	return obj;
}	
...
OBJ_DATA * make_rand_ring( sh_int level )
{
	...
	sprintf( buf, "%s %s ring", rand_material_table[material].name, rand_ring_table[type].name  );
	free_string( obj->name );
	obj->name = str_dup( buf );
	...
	rand_ring_table[type].name[0]=='u' )
		sprintf( buf, "an %s %s ring", rand_ring_table[type].name, rand_material_table[material].name );
	else
		  sprintf( buf, "a %s %s ring", rand_ring_table[type].name, rand_material_table[material].name );
	free_string( obj->short_descr );
	...
	rand_ring_table[type].name[0]=='o' ||
	rand_ring_table[type].name[0]=='u' )
		sprintf( buf, "An %s %s ring lies on the ground here.", rand_ring_table[type].name, rand_material_table[material].name );
	else
		  sprintf( buf, "A %s %s ring lies on the ground here.", rand_ring_table[type].name, rand_material_table[material].name );
	
	free_string( obj->description );
	obj->description = str_dup( buf );
	for( x=0;x<3;x++)
	obj->value[x] = (rand_material_table[material].coolness + (level/5)+ rand_ring_table[type].coolness ) / 10;
	obj->value[3] = (rand_material_table[material].coolness + (level/5)+ rand_ring_table[type].coolness ) / 4;
	...
	obj->level = level;
	   ...
	   case 0:
	      paf->location  = attrib_types[number_range(0, TABLESIZE(attrib_types)-1)];
	      paf->modifier  = number_range(1, 1+(rand_ring_table[type].coolness/2));
	      break;
	   case 1:
	      paf->location  = power_types[number_range(0, TABLESIZE(power_types)-1)];
		if(paf->location==APPLY_AC) paf->modifier *= -1;
	   ...
	   case 3:
	      paf->location  = combat_types[number_range(0, TABLESIZE(combat_types)-1)];
	      paf->modifier  = number_range(1, (level/8)+(rand_ring_table[type].coolness/3)+1);
	      break;
		}
	return obj;
}
...
OBJ_DATA * make_rand_armor( sh_int level, bool ismagic )
{
	...
	sprintf( buf, "%s %s", rand_material_table[material].name, rand_armor_table[type].name  );
	free_string( obj->name );
	obj->name = str_dup( buf );
...
rand_armor_table[type].name ); break;
	case PREF_NONE:
...
rand_armor_table[type].p_type == PREF_A)
	switch(number_range(1,3) )
		{
		case 1:sprintf( buf, "An %s %s lies on the ground here.",
			 rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 2:sprintf( buf, "An %s %s has been dropped here.",
			 rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 3:sprintf( buf, "An %s %s is resting here.",
			 rand_material_table[material].name, rand_armor_table[type].name  ); break;
		}
	else if(rand_armor_table[type].p_type == PREF_A)
	{
		switch(number_range(1,3) )
		{
		case 1:sprintf( buf, "A %s %s lies on the ground here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 2:sprintf( buf, "A %s %s has been dropped here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 3:sprintf( buf, "A %s %s is resting here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		}
	}
	else if(rand_armor_table[type].p_type == PREF_SOME)
	{
		...
		{
		case 1:sprintf( buf, "Some %s %s lie on the ground here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 2:sprintf( buf, "Some %s made out of %s lie here.",
			rand_armor_table[type].name, rand_material_table[material].name  ); break;
		case 3:sprintf( buf, "Some %s %s are resting here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 4:sprintf( buf, "Some %s %s have been dropped here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;	
		}
	}
	else if(rand_armor_table[type].p_type == PREF_NONE)
	{
		switch(number_range(1,3) )
		{
		case 1:sprintf( buf, "%s %s lies on the ground here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 2:sprintf( buf, "%s %s is resting here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;
		case 3:sprintf( buf, "%s %s has been dropped here.",
			rand_material_table[material].name, rand_armor_table[type].name  ); break;	
		}
	}
	else if(rand_armor_table[type].p_type == PREF_AN)
		sprintf( buf, "An %s made of %s lies here.", rand_armor_table[type].name, rand_material_table[material].name);
	else
	{
	sprintf(buf, "Buggy long description.  Tell an IMP if you see this description.");
	sprintf(logbuf,"make_rand_obj Long Desc bug, material=%s, type=%s. ", rand_material_table[material].name, rand_armor_table[type].name);
	log_string(logbuf);
	}
	free_string( obj->description );
	buf[0]=UPPER(buf[0]);
	obj->description = str_dup( buf );
	for(x=0;x<3;x++)
	obj->value[x] = (rand_material_table[material].coolness + rand_armor_table[type].protect_mod + (level/2)) / 4;
	obj->value[3] = (rand_material_table[material].coolness + rand_armor_table[type].protect_mod + (level/2)) / 6;
	obj->level = UMIN( level, number_fuzzy(rand_material_table[material].level) );
	 ...
	 * but I couldn't find the name of who wrote it.  Anyone know? 
	 */
	static int attrib_types[] = { APPLY_STR, APPLY_STR, APPLY_DEX, APPLY_INT,
