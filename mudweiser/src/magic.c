...
{
	 damage( ch, (CHAR_DATA *) vo, dice(2, 6) + level / 3, sn, DAM_HARM );
	 return;
}
...
{
	 damage( ch, (CHAR_DATA *) vo, dice(3, 9) + level - 3, sn, DAM_HARM );
	 return;
}
...
{
	 damage( ch, (CHAR_DATA *) vo, dice(3, 9) + level / 2, sn, DAM_HARM );
	 return;
}
...
void spell_cure_blindness( int sn, int level, CHAR_DATA *ch, void *vo )
{
	 ...
	 if (check_dispel(level,victim,gsn_blindness))
	 {
		  ...
		  act("$n is no longer blinded.",victim,NULL,NULL,TO_ROOM);
		  if (victim != ch) gain_exp(ch, hit_xp_compute( ch, ch, ch->level, 1, ch->level)/2 );
	 }
	 else
		  send_to_char("Spell failed.\n\r",ch);
}
...
void spell_cure_critical( int sn, int level, CHAR_DATA *ch, void *vo )
{
	 ...
	 heal = dice(3, 9) + level - 3;
	 victim->hit = UMIN( victim->hit + heal, victim->max_hit );
	 update_pos( victim );
	 send_to_char( "You feel better!\n\r", victim );
	 if ( ch != victim ) {
	send_to_char( "Ok.\n\r", ch );
		  gain_exp(ch, 1.25 * hit_xp_compute( ch, ch, ch->level, 1, UMIN(heal, victim->max_hit - victim->hit) ) );
	 }
	 return;
}
...
void spell_cure_disease( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	act("$n looks relieves as $s sores vanish.",victim,NULL,NULL,TO_ROOM);
			if ( ch != victim ) gain_exp(ch, hit_xp_compute( ch, ch, ch->level, 1, ch->level)/2 );
	 }
	 else
	send_to_char("Spell failed.\n\r",ch);
}
...
void spell_cure_light( int sn, int level, CHAR_DATA *ch, void *vo )
{
	 ...
	 heal = dice(2, 6) + level / 3;
	 victim->hit = UMIN( victim->hit + heal, victim->max_hit );
	 update_pos( victim );
	 send_to_char( "You feel better!\n\r", victim );
	 if ( ch != victim ) {
	send_to_char( "Ok.\n\r", ch );
		  gain_exp(ch, 2.25 * hit_xp_compute( ch, ch, ch->level, 1, UMIN(heal, victim->max_hit - victim->hit) ) );
	 }
	 return;
}
...
void spell_cure_serious( int sn, int level, CHAR_DATA *ch, void *vo )
{
	 ...
	 heal = dice(2, 9) + level + 3 /2 ;
	 victim->hit = UMIN( victim->hit + heal, victim->max_hit );
	 update_pos( victim );
	 send_to_char( "You feel better!\n\r", victim );
	 if ( ch != victim ) {
	send_to_char( "Ok.\n\r", ch );
		  gain_exp(ch, 1.75 * hit_xp_compute( ch, ch, ch->level, 1, UMIN(heal, victim->max_hit - victim->hit) ) );
	 }
	 return;
}
...
void spell_harm( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    int dam;
	 ...
	 dam = UMAX(  20, victim->hit - dice(1,3) );
    if ( saves_spell( level, victim ) )
	dam = UMIN( 50, dam / 2 );
    dam = UMIN( 100, dam );
...
void spell_heal( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	send_to_char( "A healing power flows through you.\n\r", ch );
        gain_exp(ch, hit_xp_compute( ch, ch, ch->level, 1, UMIN(100, victim->max_hit - victim->hit) ) );
    }
    return;
}
...
/* Rage is an evil version of frenzy, used for Anti-Paladins  */
/* Coded by Trilex */

/*void spell_rage(int sn, int level, CHAR_DATA *ch, void *vo)
{
	 CHAR_DATA *victim = (CHAR_DATA *) vo;
	 AFFECT_DATA af;
	...
	return;
	 ...
	 send_to_char("You are filled with insane rage!\n\r",victim);
	 act("$n gets a wild look in $s eyes!",victim,NULL,NULL,TO_ROOM);
}     */
...
void spell_refresh( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	send_to_char("You feel fully refreshed!\n\r",victim);
	 ...
	 if ( ch != victim ) {
	send_to_char( "Ok.\n\r", ch );
		  gain_exp(ch, hit_xp_compute( ch, ch, ch->level, 1, ch->level )/2 );
	 }
	 return;
}
...
act("A magic missile erupts from $n's hand and hits $N in the chest.",
	...
	ch,NULL,tmp_vict,TO_VICT);
		 ...
		 if (saves_spell(level,tmp_vict))
		dam /= 3;
		 damage(ch,tmp_vict,dam,sn,DAM_ENERGY);
		 num_missile -= 1;  /* decrement number of missiles */
	  }
	}   /* end target searching loop */
...
void spell_ice_storm(int sn, int level, CHAR_DATA *ch, void *vo)
{
	...
	dam /= 2;
		 ...
		 damage(ch,tmp_vict,dam,sn,DAM_COLD);
		 num_blast -= 1;  /* decrement number of blasts */
	  }
	}   /* end target searching loop */
