...
void move_char( CHAR_DATA *ch, int door, bool follow )
{
	...
	if ((IS_SET(to_room->room_flags, ROOM_MAGE_GUILD) && (ch->class != CLASS_OLD_MAGE && ch->class != CLASS_NEW_MAGE)) ||
(IS_SET(to_room->room_flags, ROOM_CLERIC_GUILD) && ch->class != CLASS_CLERIC) ||
(IS_SET(to_room->room_flags, ROOM_THIEF_GUILD) && ch->class != CLASS_THIEF) ||
(IS_SET(to_room->room_flags, ROOM_WARRIOR_GUILD) && ch->class != CLASS_WARRIOR) ||
(IS_SET(to_room->room_flags, ROOM_PALADIN_GUILD) && ch->class != CLASS_PALADIN) ||
(IS_SET(to_room->room_flags, ROOM_AVENGER_GUILD) && ch->class != CLASS_AVENGER) ||
(IS_SET(to_room->room_flags, ROOM_NIGHTBLADE_GUILD) && ch->class != CLASS_NIGHTBLADE))
	{
		send_to_char("Your class is not allowed in there.\n\r", ch);
		return;
	}
/*
...
void do_wake( CHAR_DATA *ch, char *argument )
{
   ...
   if (victim->pcdata)
	...
	if (victim->pcdata->message != NULL) {
	   sprintf(buf, "You have `W%d message%s`w waiting, type `Wmessages`w to read them.\n\r",
		   victim->pcdata->messages, (victim->pcdata->messages > 1) ? "s" : "" );
