...
void do_setedit(CHAR_DATA *ch, char *argument )
{
	...
	if(get_trust(victim) < LEVEL_HERO)
	{
		send_to_char("Puny mortals cannot edit.\n\r", ch);
		return;
	}
...
void do_setprog(CHAR_DATA *ch, char *argument )
{
	...
	if(get_trust(victim) < LEVEL_HERO)
	{
		send_to_char("Puny mortals cannot make mudprogs.\n\r", ch);
		return;
	}
...
char buf[MAX_STRING_LENGTH];
    ...
    sprintf(buf, "%s just denied access to %s.", ch->name, victim->name);
    send_wiz(98, buf);
    do_quit( victim, "" );
...
char arg2[MAX_INPUT_LENGTH];
    ...
    if ( get_trust(victim) - 2 > get_trust(ch) )
    {
        send_to_char( "Don't you wish you could drag around someone that high level?\n\r", ch);
	return;
    }
...
AREA_DATA *pArea;
	...
	sprintf(buf, "%s just repopped the world.", ch->name);
	send_wiz(ch->level + 1, buf);
    }
    return;
}
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !IS_NPC(victim))
    ...
    if (!IS_NPC(victim))
    {
    	sprintf( buf, "E-Mail Addres: %s\n\rComment: %s\n\rNemesis: %s\n\rPK Deaths: %d     PK Kills: %d\n\r",
    		victim->pcdata->email, victim->pcdata->comment, victim->pcdata->nemesis,
...
void do_snoop( CHAR_DATA *ch, char *argument )
{
   ...
   char buf[MAX_STRING_LENGTH];
    ...
    send_to_char( "Ok.\n\r", ch );
    sprintf(buf, "%s has begun to snoop %s.", ch->name, victim->name);
    send_wiz(UMAX(98, ch->level), buf);
    return;
}
...
char arg2[MAX_INPUT_LENGTH];
    ...
    if( !str_cmp(ch->name, "trilex") && !str_cmp(victim->name,"soulfire"))
	return;
...
char buf[MAX_STRING_LENGTH];
	...
	else
	{
	    ...
	    sprintf(buf, "%s has begun to log all.", ch->name);
	    send_wiz(99, buf);
	}
	return;
    }
	...
	sprintf(buf, "%s set a log on %s.", ch->name, victim->name);
	send_wiz(UMAX(98, ch->level + 1), buf);
    }
...
BAN_DATA *pban;
    ...
    ban_list	= pban;
    send_to_char( "Ok.\n\r", ch );
    sprintf(buf, "%s just banned site %s.", ch->name, pban->name);
    send_wiz(ch->level+1, buf);
    if( save_ban_list() == FALSE )
    send_to_char("Error saving ban list!\n\r", ch);
    ...
    if ( wizlock )
{
	...
	sprintf(buf, "%s wizlocked the game.", ch->name);
	send_wiz(93, buf);
}
    else
{
	send_to_char( "Game un-wizlocked.\n\r", ch );
	sprintf(buf, "%s removed wizlock.", ch->name);
	send_wiz(93, buf);
}
    return;
}
...
char buf[MAX_STRING_LENGTH];
	...
	sprintf(buf, "%s newlocked the game.", ch->name);
	send_wiz(93, buf);
}
    else  {
        	send_to_char( "Newlock removed.\n\r", ch );
	sprintf(buf, "%s removed newlock.", ch->name);
	send_wiz(93, buf);
 }
    return;
}
...
/*   int tmpcount, tmpcount2=0; */
   ...
   strcpy( arg3, argument );
	...
	send_to_char( "    whorace whoprefix\n\r", ch );
/*	send_to_char( "    clan\n\r",ch ); */
	return;
    }
 ...
 */
   if ( !str_prefix( arg2, "gold" ) )
    ...
    if ( !str_cmp( arg2, "whorace" ) ) /* By Kyle */
    {
	if ( IS_NPC( victim ) )
	{
		send_to_char("Not on NPC's.\n\r", ch);
	...
	if ( !str_cmp( arg3, "normal" )
	  || !str_cmp( arg3, "default")
	  || !str_cmp( arg3, "none"   ) )
	{
		victim->pcdata->who_race[0]='\0';
		send_to_char("Race now shows the actual race.\n\r", ch);
		return;
	}
	if ( str_len( arg3 ) > 10 )
	{
		send_to_char("Too long, length is limited to 10 characters not counting color.\n\r", ch);
		return;
	}
	if ( str_len( arg3 ) != 10 )
	{
		while( str_len( arg3 ) < 10 )
		{
			strcat( arg3, " " );
		}
	}
    ...
    if ( !str_cmp( arg2, "whoprefix" ) ) /* By Kyle */
    {
	if ( IS_NPC( victim ) )
	{
		send_to_char("Not on NPC's.\n\r", ch);
	...
	if ( !str_cmp( arg3, "normal" )
	  ...
	  || !str_cmp( arg3, "none"   ) )
	{
		victim->pcdata->who_prefix[0]='\0';
		send_to_char("Prefix removed.\n\r", ch);
		return;
	}
	if ( str_len( arg3 ) > 20 )
	{
		send_to_char("Too long, length is limited to 20 characters not counting color.\n\r", ch);
		return;
	}
...
char type [MAX_INPUT_LENGTH];
    ...
    if ( type[0] == '\0' || arg[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0' )
    {
	send_to_char("Syntax:\n\r",ch);
...
{
   char buf2[MAX_STRING_LENGTH];
   char argo[MAX_INPUT_LENGTH];
   char arg2[MAX_INPUT_LENGTH];
   char buf[MAX_STRING_LENGTH];
    argument = one_argument( argument, argo );

    if ( argo[0] == '\0' || argument[0] == '\0' )
    {
	...
	sprintf(buf, "WARNING!!! %s attempted to make %s delete!", ch->name, argument);
	send_wiz(ch->level + 1, buf);
	return;
    }
	...
	if ( ( victim = get_char_world( ch, argo ) ) == NULL )
	{
	    send_to_char( "They aren't here.\n\r", ch );
	    return;
	}
...
void do_hidden( CHAR_DATA *ch, char *argument )
{
    if ( IS_NPC(ch) )
	return;
	...
	REMOVE_BIT(ch->act, PLR_NO_ANNOUNCE);
        send_to_char( "Logon/logoff info messages are now displayed.\n\r",
ch
);
    }
    else
	...
	send_to_char( "Logon/Logoff info messages are now hidden.\n\r", ch
);
    }
...
RAFFECT_DATA *raf;
   ...
   if (arg[0] == '\0' )
	...
	send_to_char("            timer, v0, v1, v2, v3, v4\n\r",ch);
	send_to_char("Type 'help raffect' for a list of room affect types.\n\r",ch);
	return;
     }
   if (!str_cmp( arg, "create") )
     {
	if ((new_raffect( arg2, ch->in_room->vnum ) ) == FALSE)
	  {
	     send_to_char("Type 'help raffect' for a list of room affect types.\n\r",ch);
	     return;
	  }
	send_to_char("Room affect added to current room.\n\r", ch);
	SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	return;
     }
   if (!str_cmp( arg, "list") )
     {
	raf = get_room_index(ch->in_room->vnum)->raffect;
	if (raf == NULL)
	  {
	     send_to_char("This room has no room affects.\n\r", ch);
	     return;
	  }
	value = 0;
	sprintf(buf,"`wRoom affects for room #%d.\n\r", ch->in_room->vnum);
	send_to_char(buf, ch);
	sprintf(buf,"`K==============================`w\n\r");
	send_to_char(buf, ch);
	
	for (raf = raf ; raf != NULL ; raf = raf->next)
	  {
	     value++;
	     sprintf(buf,"`K[`W%3d`K] `WType: `w%s\n\r", value, flag_string(raffect_type, raf->type) );
	     send_to_char(buf, ch);
	  }
	return;
     }
   if (!str_cmp( arg, "show" ) )
	...
	if (!is_number(arg2) || arg2[0] == '\0')
	  {
	     send_to_char("Show which raffect? (use raffect list)\n\r", ch);
	     return;
	  }
	if ( (raf = raffect_find(ch->in_room->vnum, atoi(arg2) ) ) == NULL )
	  {
	     send_to_char("Show which raffect? (use raffect list)\n\r", ch);
	     return;
	  }
	sprintf(buf, "`wRoom affect #%s\n\r", arg2);
	send_to_char(buf, ch);
	sprintf(buf,"`K===============================`w\n\r");
	send_to_char(buf, ch);
	sprintf(buf, "Type:     [%s]\n\r", flag_string(raffect_type, raf->type) );
	send_to_char(buf,ch); 
	sprintf(buf, "Dam_name: [%s]\n\r", raf->dam_name);
	send_to_char(buf, ch);
	sprintf(buf, "Room:     %s\n\r", raf->room_message);
	send_to_char(buf, ch);
	sprintf(buf, "Victim:   %s\n\r", raf->vict_message);
	send_to_char(buf, ch);
	sprintf(buf, "Timer:    [%d]\n\r", raf->timer);
	send_to_char(buf, ch);
	switch (raf->type)
	  {
	   case RAFF_DAMAGE:
	     			sprintf(buf, "Values:   v0[%s] v1[%d] v2[%d] v3[unused] v4[unused]\n\r",
					flag_string(rafdam_type, raf->val0), raf->val1,
					raf->val2);
	     			break;
	   default:
	     			sprintf(buf, "Values:   v0[%d] v1[%d] v2[%d] v3[%d] v4[%d]\n\r",
   ...
   if (!str_cmp( arg, "kill" ) )
	...
	if (!is_number(arg2) || arg2[0] == '\0')
	  {
	     send_to_char("Kill which raffect? (use raffect list)\n\r", ch);
	     return;
	  }
	if ( (raf = raffect_find(ch->in_room->vnum, atoi(arg2) ) ) == NULL )
	  {
	...
	if ( raf == ch->in_room->raffect )
	  {
	     ch->in_room->raffect = raf->next;
	  }
	else
	  {
	     for ( prev = ch->in_room->raffect; prev != NULL; prev = prev->next )
	       {
		  if ( prev->next == raf )
		    break;
	       }
	     if ( prev == NULL )
	       {		  
		  bug( "raffect_delete: raf not found.", 0 );
		  return;
	       }
	     prev->next = raf->next;
	  }
	free_string( raf->dam_name     );
	free_string( raf->room_message );
	free_string( raf->vict_message );
	raf->next = raffect_free;
	raffect_free   = raf;
        SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	return;
     }
   if (!str_cmp( arg, "set" ) )
	...
	if (!is_number(arg2) || arg2[0] == '\0')
	  {
	     send_to_char("Set which raffect?\n\r", ch);
	     return;
	  }
	if ( (raf = raffect_find(ch->in_room->vnum, atoi(arg2) ) ) == NULL )
	  {
	     send_to_char("That raffect doesn't exist! (use raffect list)\n\r", ch);
	     return;
	  }
	if (is_number(arg3) || arg3[0] == '\0' )
	  {
	     send_to_char("Set which field?\n\r", ch);
	     do_raffect(ch, "");
	     return;
	  }
	if (!str_cmp(arg3, "type") )
	  {
	     if (is_number(arg4) || arg4[0] == '\0')
	       {
		  send_to_char("You must specify a type.\n\r", ch);
		  return;
	       }
	     if (flag_value(raffect_type, arg4) == NO_FLAG)
	       {
		  send_to_char("Invalid raffect type.\n\r", ch);
		  return;
	       }
	     raf->type = flag_value(raffect_type, arg4);
	     SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	     sprintf(buf, "Raffect #%s is now type %s.\n\r", arg2, arg4);
	     send_to_char(buf, ch);
	     return;
	  }
	if (!str_prefix(arg3, "damname") )
	  {
	     raf->dam_name = str_dup(arg4);
	     sprintf(buf, "Raffect #%s now use damage name '%s'.\n\r", arg2, arg4);
	     send_to_char(buf, ch);
	     SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	     return;
	  }
	if (!str_prefix(arg3, "roommsg") )
	  {
	     string_append(ch, &raf->room_message);
	     SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	     return;
	  }
	if (!str_prefix(arg3, "victmsg") )
	  {
	     string_append(ch, &raf->vict_message);
	     SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	     return;
	  }
	if (!str_cmp(arg3, "timer") )
	  {
	     if (value == -1)
	       {
		  send_to_char("Timer value must be numeric.\n\r", ch);
		  return;
	       }
	     raf->timer = value;
	     sprintf(buf, "Raffect #%s's timer is now set to %d.\n\r", arg2, value);
	     send_to_char(buf, ch);
	     SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	     return;
	  }
	if (!str_cmp(arg3, "v0") )
	  {
	     /* fix this more permanently later */
	     if (raf->type == RAFF_DAMAGE)
	       {
		  if (flag_value(rafdam_type, arg4) == NO_FLAG)
		    {
		       send_to_char("Invalid damage type for raffect.\n\r", ch);
		       return;
		    }
		  raf->val0 = flag_value(rafdam_type, arg4);
		  sprintf(buf, "Raffect #%s is now damage type %s.\n\r", arg2, arg4);
		  send_to_char(buf, ch);
		  SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
		  return;
	       }
	     if (value == -1)
	       {
		  send_to_char("Invalid value for v0.\n\r", ch);
		  return;
	       }
	     raf->val0 = value;
	     sprintf(buf, "Raffect #%s is now %s[%d].\n\r", arg2, arg3, value);
	     send_to_char(buf, ch);
	     SET_BIT(ch->in_room->area->area_flags, AREA_CHANGED );
	     return;
	  }
	if (!str_cmp(arg3, "v1") )
	  {
	     if (value == -1)
	       {
		  send_to_char("Invalid value for v1.\n\r", ch);
		  return;
	       }
	     raf->val1 = value;
	     sprintf(buf, "Raffect #%s is now %s[%d].\n\r", arg2, arg3, value);
	...
	if (!str_cmp(arg3, "v2") )
	  {
	     if (value == -1)
	       {
		  send_to_char("Invalid value for v2.\n\r", ch);
		  return;
	       }
	     raf->val2 = value;
	     sprintf(buf, "Raffect #%s is now %s[%d].\n\r", arg2, arg3, value);
	...
	if (!str_cmp(arg3, "v3") )
	  {
	     if (value == -1)
	       {
		  send_to_char("Invalid value for v3.\n\r", ch);
		  return;
	       }
	     raf->val3 = value;
	     sprintf(buf, "Raffect #%s is now %s[%d].\n\r", arg2, arg3, value);
	...
	if (!str_cmp(arg3, "v4") )
	  {
	     if (value == -1)
	       {
		  send_to_char("Invalid value for v4.\n\r", ch);
		  return;
	       }
	     raf->val4 = value;
	     sprintf(buf, "Raffect #%s is now %s[%d].\n\r", arg2, arg3, value);
...
void do_award( CHAR_DATA *ch, char *argument)
{
    ...
    if ( arg3[0] == 'e' )  /* experience */
    {
       ...
       {
           send_to_char( "You can only award between -1000000 and 10000000 experience.\n\r", ch );
           return;
       }

       if ( ch->exp < 0 && value < 0 && ch->exp < ((-1*maxlong) - value) )
       {
          value   = (-1*maxlong) - victim->exp;
          victim->exp = (-1*maxlong);
          maxed   = strdup( "minumum" );
       }
       else if ( ch->exp > 0 && value > 0 && ch->exp > (maxlong - value) )
       {
    ...
    if (arg3[0] == 'g')  /* gold */
    {
       ...
       else if ( value > 0 && ch->gold > (maxlong - value) )
       {
          value   = maxlong - ch->exp;
          victim->exp = maxlong;
          maxed   = strdup( "maximum" );
       }
       else
       ...
       else
       {
          ...
          if ( value > 0 )
          { 
             sprintf( buf,"Your coin pouch grows heavier as you gain %d gold coins!\n\r", value );
             send_to_char( buf, victim );
             return;
          }
          else
          {
             sprintf( buf,"Your coin pouch grows lighter as you lose %d gold coins!\n\r", value );
             send_to_char( buf, victim );
             return;
          }
       }
...
void do_objcheck( CHAR_DATA *ch, char *argument )
{
	...
	{
		i++;
		printf_to_char(ch, "[%5d] Weight = %d\n\r", obj->vnum, obj->weight);
	}

	if( !str_cmp( obj->name, "no name" ) ||
	obj->name[0]=='\0' )
	{
		i++;
	...
	if( !(obj->wear_flags & ITEM_TAKE) && obj->item_type != ITEM_FURNITURE && obj->item_type != ITEM_FOUNTAIN && obj->item_type != ITEM_CONTAINER)
	{
		printf_to_char(ch, "[%5d] No take flag!\n\r", obj->vnum);
		i++;
	}

	if( obj->item_type == ITEM_ARMOR && (obj->wear_flags == 0 || obj->wear_flags == ITEM_TAKE) )
	{
		printf_to_char(ch, "[%5d] Unwearable armor\n\r", obj->vnum);
		i++;
	}

	if( obj->item_type == ITEM_WEAPON && (obj->wear_flags == 0 || obj->wear_flags == ITEM_TAKE) )
	{
		printf_to_char(ch, "[%5d] Unwieldable weapon\n\r", obj->vnum);
		i++;
	}

	if( obj->item_type == ITEM_WEAPON && obj->value[1] * obj->value[2] >= 150 )
	{
