...
*/
    ...
    ||   ( IS_OBJ_STAT(obj, ITEM_ANTI_MAGE) && ch->class == CLASS_OLD_MAGE && !IS_NPC(ch) )
    ...
    ||   ( IS_OBJ_STAT(obj, ITEM_ANTI_AVENGER) && ch->class == CLASS_AVENGER )
    ||   ( IS_OBJ_STAT(obj, ITEM_ANTI_NIGHTBLADE) && ch->class == CLASS_NIGHTBLADE )
    ||   ( IS_OBJ_STAT(obj, ITEM_ANTI_MAGE) && ch->class == CLASS_NEW_MAGE )  
...
char *extra_bit_name( int extra_flags )
{
   char buf[MAX_STRING_LENGTH]; 
    ...
    if ( extra_flags & ITEM_ANTI_PALADIN ) strcat( buf, " anti-paladin"    );
    if ( extra_flags & ITEM_ANTI_AVENGER ) strcat( buf, " anti-avanger"    );
    if ( extra_flags & ITEM_ANTI_NIGHTBLADE ) strcat( buf, " anti-nightblade"    );
...
char *act_bit_name( int act_flags )
{
   char buf[MAX_STRING_LENGTH]; 
	...
	if (act_flags & PLR_CAN_PROG    ) strcat(buf, " can_program");
    }
    return ( buf[0] != '\0' ) ? buf+1 : "none";
}
...
RAFFECT_DATA *raffect_find(int vnum, int slot)
{
   RAFFECT_DATA *raf_find;
   int sn;
   
   raf_find = get_room_index(vnum)->raffect;
   
   if (raf_find == NULL) return NULL;
   
   
   for ( sn = 1; sn != slot; sn++)
     {
	if (raf_find->next == NULL) return NULL;
	raf_find = raf_find->next;
     }
   return raf_find;
}


bool new_raffect (char *type, int vnum)
{
   RAFFECT_DATA *raf;
   RAFFECT_DATA *pRaf;
   sh_int itype;
   ...
   if ( (itype = flag_value(raffect_type, type) ) == NO_FLAG)
     {
	return FALSE;
     }
   ...
   raf->dam_name = str_dup("nothing");
   raf->room_message = str_dup("$n is affected by the room affect in this room!");
   raf->vict_message = str_dup("You are affected by the room affect in this room!");
   raf->timer = 1;		         /* how often the affect goes off */
   raf->val0 = 0;		         /* data for the raffect */
   raf->val1 = 0;		         /* data for the raffect */
   raf->val2 = 0;		         /* data for the raffect */
   raf->val3 = 0;
   raf->val4 = 0;
   ...
   if (rix->raffect == NULL)      /* if this is the first room affect for this room */
	...
	/* need to goto the last affect on the list first. - Thexder */
