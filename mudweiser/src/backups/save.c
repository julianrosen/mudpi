...
/***/
	...
	fprintf( fp, "PKdi %d\n", ch->pcdata->pk_deaths);
	fprintf( fp, "PKki %d\n", ch->pcdata->pk_deaths);
	if ( ch->pcdata->who_race[0] != '\0' )
	fprintf( fp, "WRace %s~\n", ch->pcdata->who_race);
	if ( ch->pcdata->who_prefix[0] != '\0' )
...
/***/
   ...
   ch->pcdata->recall_room		= get_room_index( ROOM_VNUM_TEMPLE );
    ...
    ch->pcdata->who_race		= str_dup( "" );
    ch->pcdata->who_prefix		= str_dup( "" );
    found = FALSE;
    fclose( fpReserve );
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
   char buf[MAX_STRING_LENGTH];
	...
	case 'E':
		...
		if (ch->desc->ansi && !fCopyOver) SET_BIT(ch->act,PLR_COLOR);
		else if (!fCopyOver) REMOVE_BIT(ch->act,PLR_COLOR);
		else if (IS_SET(ch->act,PLR_COLOR)) ch->desc->ansi = TRUE;
		else ch->desc->ansi = FALSE;
		return;
	     }
