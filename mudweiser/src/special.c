...
DECLARE_SPEC_FUN( spec_french_taunter	);
DECLARE_SPEC_FUN(	spec_sea_captain		);
DECLARE_SPEC_FUN(	spec_reaper		);
DECLARE_SPEC_FUN( spec_tim		);
DECLARE_SPEC_FUN( spec_nee		);
/*
...
/*    { "spec_clan_guard",	spec_clan_guard		}, */
    ...
    { "spec_lumberjack",	spec_lumberjack	},
    { "spec_french_taunter", 	spec_french_taunter	},
    { "spec_sea_captain",	spec_sea_captain		},
    { "spec_reaper",		spec_reaper		},
    { "spec_tim",			spec_tim		},
    { "spec_nee",			spec_nee		},
    /*
     * End of list.
...
char *spec_string( SPEC_FUN *fun )	/* OLC */
{
    ...
    for ( cmd = 0; spec_table[cmd].spec_fun(0) != '\0'; cmd++ ) // JR: changed [0] to (0)
...
bool spec_lumberjack( CHAR_DATA *ch )
{
	...
	silliness = number_range (1, 50);
		
	if ( silliness <= 40)
		return TRUE;
	else if( silliness <= 45)
		do_say ( ch, "I'm a lumberjack, and I'm ok...");
	else 
		do_emote( ch, "hums quietly while chopping some wood.");

	return TRUE;
}

bool spec_sea_captain( CHAR_DATA *ch )
{
	int silliness;
	...
	silliness = number_range (1, 20);
		
	if ( silliness <= 10)
		return TRUE;
	else if( silliness <= 18)
		do_say( ch, "Die, landlubbers!");
	else 
		do_say( ch, "Arrrr.....");
...
bool spec_reaper( CHAR_DATA *ch )
{
	...
	silliness = number_range (1, 40);
		
	if ( silliness <= 20)
		return TRUE;
	else if( silliness <= 30)
		do_say( ch, "Your fate lies in my hands, you fool!");
	else 
		do_say( ch, "`RMuahahahahaha!!");

	return TRUE;
}

bool spec_french_taunter( CHAR_DATA *ch )
{
	static int taunt;
 ...
 *	     ch, NULL, NULL, TO_ROOM );
 *	 sprintf(buf, "Quick, %s is fighting %s!!", ech->name, ech->fighting->name);
 *	 do_yell(ch, buf);
 *	 multi_hit( ch, ech, TYPE_UNDEFINED );
 *	 return TRUE;
 *   }
...
bool spec_tim( CHAR_DATA *ch )
{
	if (ch->position==POS_FIGHTING && ch->fighting != NULL)
	{
		if(number_range(0,2)==0 && !IS_NPC(ch->fighting))
		{
			act("$N points at $n and shoots a fireball.", ch->fighting, NULL, ch, TO_ROOM);
			send_to_char("Tim blasts you with a fireball!\n\r", ch->fighting);
			do_cast(ch, "'fireball'");
		}
	return TRUE;
	}
...
bool spec_nee( CHAR_DATA *ch )
{
	...
	if (ch->position!=POS_FIGHTING && ch->position > POS_SLEEPING)
	{
		...
		do_say(ch, "Nee!"); return TRUE;
		case 4: do_say(ch, "We require a shrubbery!"); return TRUE;
		default: return FALSE;
		}
	}
	else if(ch->position==POS_FIGHTING && ch->fighting != NULL && number_range(0,2)==0)
	{
		dam=number_range(15,25);
		act("$N looks at $n and says, \"NEE!\"\n\r$n staggers back from the vicious NEE-ing.", ch->fighting, NULL, ch, TO_ROOM);
		sprintf(buf, "%s says, \"NEE!\" at you!\n\rYou take %d damage!\n\r", ch->short_descr, dam);
